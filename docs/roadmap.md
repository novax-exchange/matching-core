...
| Matching runtime | Completed for current stage | `matching_runtime.rs`, `matching_runtime_driver.rs`; configured execution mode, input preflight, input close / drain / shutdown boundaries, shard status, aggregate run stop reasons, and manual multi-shard execution are covered for the current layer |

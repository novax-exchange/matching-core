...
|---|---|
| 已完成阶段 | Phase 0-18 |
| 当前里程碑 | 可线程化 Runtime Loop |
| 下一阶段 | Phase 19：Output isolation |
| 最新验证方式 | `cargo test` |
...
- Runtime loop step 和 one-shot worker thread，用于处理队列中的 symbol input。
...
| 18 | 已完成 | 引入线程模型 | Journal reader 与 runtime 分离 |
| 19 | 下一步 | 实现 output isolation | 慢输出不直接阻塞输入读取 |
